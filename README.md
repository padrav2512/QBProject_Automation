...
Mapping preparation reads per-question `Curriculum:` and `Taxonomy:` blocks, removes those author-only lines from the processed DOCX and creates a mapping CSV. After the snippets exist in CMS, that CSV can be uploaded in the app's **Apply mappings to CMS** area. A separate mapping-only task may use either the long columns `Question`, `Snippet ID`, `Mapping Type`, `Path`, or the team's wide columns `Project with qno`/`Project`, optional `Snippet ID`, `Taxonomy`, `Curriculum`. Multiline Curriculum and Taxonomy cells are expanded to one path per row. A snippet ID or an exact CMS question name is sufficient; when both are supplied, the app verifies that they agree. Project names, column headings and Curriculum/Taxonomy paths are matched case-insensitively; spelling and hierarchy remain significant. When an XLSX has several worksheets, the user selects the worksheet containing the mapping rows. The mapper only adds missing paths; existing mappings are retained and never removed or replaced. When a path is absent, the pre-check shows up to three hierarchy-aware suggestions from the live CMS tree and never applies a suggestion automatically.
